...
popover_contents <- readRDS(url("http://jilab.biostat.jhsph.edu/software/PDDB/test-app/popover_contents.rds"))

# Get genomic ranges from file
genomic_ranges <- read.delim("http://jilab.biostat.jhsph.edu/software/PDDB/test-app/BIRD_output_ranges.txt")
...
gbin_tss <- read.delim('http://jilab.biostat.jhsph.edu/software/PDDB/test-app/gbin_tss.txt')

# Get data.frame matching genes to their TSS's
gene_tss <- read.delim('http://jilab.biostat.jhsph.edu/software/PDDB/test-app/gene_tss.txt')

# Get data.frame matching diseases/traits to gbins
snp_gbin <- read.delim('http://jilab.biostat.jhsph.edu/software/PDDB/test-app/snp_gbin.txt')

# Get data.frame matching gbins to diseases/traits
gbin_snp <- read.delim('http://jilab.biostat.jhsph.edu/software/PDDB/test-app/gbin_snp.txt')

# Get data.frame of overall sample prediction means
sample_avgs <- read.delim('http://jilab.biostat.jhsph.edu/software/PDDB/test-app/sample_avgs.txt')

# Set recount3 url
recount3_url <- "https://recount-opendata.s3.amazonaws.com/recount3/release"

# Read h5 files mapping to studies
h5_studies <- read.delim('http://jilab.biostat.jhsph.edu/software/PDDB/test-app/h5_studies.txt')
s3 <- TRUE
h5_url <- "https://s3.us-east-2.amazonaws.com/raina.test.bucket/BIRD_predictions.h5"

# Read samples table
all_samples_df <- read.delim('http://jilab.biostat.jhsph.edu/software/PDDB/test-app/samples_df.txt')

# Read projects table
proj_df <- read.delim('http://jilab.biostat.jhsph.edu/software/PDDB/test-app/proj_df.txt')
...
ui <- fluidPage(
  ...
  navbarPage(
    ...
    navbarMenu(
      ...
      tabPanel(
        title = "txt/zip download", 
        
        div(style = "display: inline-block;vertical-align: middle;", h2("txt/zip Download")), 
        div(style = "display: inline-block;vertical-align: middle;", 
            bsButton(
...
server <- function(input, output, session) {
  ...
  samples_df <- reactive({
    ...
    all_samples_df[all_samples_df$project_id %in% selected_proj, ]
  })
  
  # Render output samples table
  output$sample_table <- DT::renderDataTable(samples_df(),
                                             rownames = FALSE, 
                                             filter = list(position = 'top', clear = FALSE))
  ...
  get_all_pred_df <- function(custom_range) {
    predict_proj <- selected_samples()$project
    predict_sample <- selected_samples()$sample
    predict_source <- selected_samples()$file_source
    # Define custom range in terms of BIRD genomic bins
    hits <- findOverlaps(bird_ranges, custom_range)
    custom_range <- bird_ranges[sort(unique(hits@from))]
    pred_df <- data.frame(Chromosome = seqnames(custom_range), 
                          Start = start(custom_range), 
                          End = end(custom_range))
    pred_matrix <- sapply(1:nrow(selected_samples()), function(i) {
      proj_id <- predict_proj[i]
      sample_id <- predict_sample[i]
      file_source <- predict_source[i]
      if (!is.character(file_source)) {
        print(file_source)
      }
      if (startsWith(file_source, "user_txt_upload/")) {
        gr <- user_txt_gr_list()[[proj_id]]
        pred <- user_txt_pred_df()[, sample_id]
        # Fit prediction to custom range
        hits <- findOverlaps(custom_range, gr)
        pred_vec <- rep(0, length(custom_range))
        pred_vec[hits@from] <- pred[hits@to]
        return(pred_vec)
      }
      if (file_source == 'user_bam_upload') {
        hits <- findOverlaps(custom_range, bird_ranges)
        return(user_bam_pred_df()[sort(unique(hits@to)), sample_id])
      }
      get_pred_vec(proj_id, sample_id, custom_range)
    })
    if (length(custom_range) == 1) {
      pred_matrix <- matrix(pred_matrix, nrow = 1)
    }
    colnames(pred_matrix) <- predict_sample
    return(cbind(pred_df, pred_matrix))
  }
  ...
  output$download_bw_ui <- renderUI({
    if (nrow(selected_samples()) == 0) {
      ...
      tagList(
        ...
        p("Under construction - add UCSC track link")
      )
    }
  })
  ...
  output$bw_download <- downloadHandler(
    ...
    content = function(file) {
      ...
      tmp <- tempdir()
      tmp_fpaths <- c()
      pred_df <- get_all_pred_df(bird_ranges)
      pred_df[, 4:ncol(pred_df)] <- sapply(pred_df[, 4:ncol(pred_df)], as.numeric)
      pred_df[, 4:ncol(pred_df)] <- 2 ^ pred_df[, 4:ncol(pred_df)] - 1
      # Convert predictions data.frame to string
      pred_str <- ""
      for (i in 1:nrow(pred_df)) {
        pred_str <- paste0(pred_str, paste(pred_df[i, ], collapse='\t'), '\n')
      }
      bg <- import(text = pred_str, format = "bedGraph")
      seqlevels(bg) <- paste0('chr', seqlevels(bg))
      bsg <- BSgenome.Hsapiens.UCSC.hg38
      seqlevels(bg) <- seqlevels(bsg)
      seqinfo(bg) <- seqinfo(bsg)
      for (i in seq_along(names(bg@elementMetadata@listData))) {
        gr <- GRanges(seqnames = bg@seqnames, 
                      ranges = bg@ranges, 
                      seqinfo = bg@seqinfo, 
                      score = bg@elementMetadata@listData[[i]])
        fname <- paste0(colnames(pred_df)[3 + i], ".bw")
        export.bw(gr, file.path(tmp, fname))
        tmp_fpaths <- append(tmp_fpaths, file.path(tmp, fname))
      }
      zip(zipfile = file, files = tmp_fpaths, extras = '-j')
    }
  ...
  output$pca_cumvar_plot <- renderPlot({
    ...
    cumvar <- cumvar[paste0('PC', 1:min(5, nrow(cumvar))), ]
        ...
        mainPanel(
          selectInput(
            ...
            choices = c("Pseudo-time Trajectory", 
                        "Genomic Bin Clusters Chromatin Accessibility Along Pseudotime", 
                        "Individual Genomic Bin Chromatin Accessibility Along Pseudotime", 
                        "Gene Average Chromatin Accessibility Along Pseudotime"), 
  ...
  output$pt_gbin_chracc_ui <- renderUI({
    if (input$pt_gbin_chracc_choose_plot == "Scatterplot") {
      ...
      tagList(
        h4("Genomic bin chromatin accessibility along pseudo-time heat map"), 
        fluidRow(
          column(
            6, 
            radioButtons(
  ...
  pca_top_var_nearest_genes <- reactive({
    gbin_names <- rownames(pca_top_var_pred_mat())
    nearest_genes <- unique(gbin_tss[gbin_names, ]$gene)
  ...
  output$pt_ui <- renderUI({
    ...
    } else if (input$pt_show_panel == "Genomic Bin Clusters Chromatin Accessibility Along Pseudotime") {
      tagList(
    ...
    } else if (input$pt_show_panel == "Individual Genomic Bin Chromatin Accessibility Along Pseudotime") {
      tagList(
    ...
    } else if (input$pt_show_panel == "Gene Average Chromatin Accessibility Along Pseudotime") {
      tagList(
  ...
  pt_diff_test_res <- reactive({
    ...
    if (input$pt_diff_show_panel == "Individual genomic bins") {
      ...
      gbin_names <- rownames(pca_top_var_pred_mat())
      nearest_genes <- gbin_tss[gbin_names, ]$gene
      gene_avg <- aggregate(pca_top_var_pred_mat(), 
                            by=list(gene=nearest_genes), 
                            mean)
  ...
  pt_diff_go_sig_bins <- reactive({
    if (input$pt_diff_show_panel == "Genomic bin clusters") {
      ...
      significant_bins <- pt_diff_sig_bins()
    }
    significant_bins
  })
    ...
    if (!is.null(pt_diff_test_res())) {
      if (input$pt_diff_show_panel == "Individual genomic bins") {
        ...
        return(paste("You selected", 
                     length(sig_clusts), 
                     "significant clusters, which maps to", 
                     length(sig_genes), 
                     "nearest genes that will be used in GO term enrichment analysis. "))
      } else if (input$pt_diff_show_panel == "Gene average accessibility") {
      ...
      tagList(
        selectInput(
          ...
          c('Top GO terms table',
            ...
            'Volcano plot',
            'GO terms bar plot', 
            'Significant genes accessibility along pseudo-time')
        ),
  ...
  output$pt_diff_go_res_ui <- renderUI({
    if (input$pt_diff_go_sel_res == 'Top GO terms table') {
      ...
      tagList(
        ...
        plotlyOutput("pt_diff_go_barplot"),
        bsTooltip("pt_diff_go_barplot",
                  title = "You can zoom in on the bar plot using the zoom tool. ",
                  placement = "top",
                  options = list(container = "body"))
      )
    } else if (input$pt_diff_go_sel_res == "Significant genes accessibility along pseudo-time") {
          ...
          fluidRow(
            ...
            column(
              ...
              radioButtons(
                ...
                "Show plot: ", 
                choices = c("Trend lines", 
                            "Scatter plot with overall trend")
              )
            ),
            column(
              4, 
  ...
  output$pt_diff_go_all_chracc_plot <- renderPlotly({
    if (input$run_pt_diff_go) {
      fit_vals <- apply(pt_diff_top_go_terms(), 1, function(x) {
        ...
        gbin_genes <- pt_diff_go_sig_bins_table()[pt_diff_go_sig_bins_table()$nearest_gene %in% genes, , drop=F]
  ...
  output$pt_diff_go_chracc_overview_ui <- renderUI({
    if (input$pt_diff_go_chracc_plot == "Trend lines") {
      ...
      header_text <- paste(input$pt_diff_go_chracc_form, "average accessibility along pseudo-time scatter plot")
  ...
  # Render GO bar plot
  output$pt_diff_go_barplot <- renderPlotly({
    if (! is.null(pt_diff_top_go_terms())) {
      ...
      g <- g + xlab("GO terms")
      if (input$pt_diff_go_barplot_var == "p-value") {
        g <- g + ylab('- log<sub>10</sub> p-value')
        g <- g + labs(fill = '- log<sub>10</sub> p-value')
        g <- g + geom_bar(stat = 'identity', width = 0.5) + coord_flip() + theme_bw()
        p <- ggplotly(g) %>% style(hoverinfo = 'none')
      } else if (input$pt_diff_go_barplot_var == "FDR") {
        g <- g + ylab('- log<sub>10</sub> FDR')
        g <- g + labs(fill = '- log<sub>10</sub> FDR')
        g <- g + geom_bar(stat = 'identity', width = 0.5) + coord_flip() + theme_bw()
        p <- ggplotly(g) %>% style(hoverinfo = 'none')
      }
      p
  ...
  output$diff_go_sig_genes_msg <- renderText({
    if (!is.null(diff_test_res())) {
      ...
      sig_genes <- unique(gbin_tss[significant_bins, ]$gene)
      sig_genes <- sig_genes[! is.na(sig_genes)]
      paste("There are ", 
  ...
  output$diff_go_res_ui <- renderUI({
    if (input$diff_go_sel_res == 'Top GO terms table') {
      ...
      tagList(
        ...
        plotlyOutput("diff_go_barplot"), 
        bsTooltip("diff_go_barplot", 
                  title = "You can zoom in on the bar plot using the zoom tool. ", 
                  placement = "top", 
                  options = list(container = "body"))
      )
  ...
  # Render GO bar plot
  output$diff_go_barplot <- renderPlotly({
  ...
  disease_gbins_heat_map <- reactive({
    ...
    if (input$disease_heatmap_snp_subset && (! is.null(input$disease_heatmap_snp_sel))) {
