...
genomic_ranges <- read.table("BIRD_output_ranges.txt", header = TRUE, sep = "\t")
...
gbin_tss <- read.table('gbin_tss.txt', header = TRUE, sep = '\t')

# Get data.frame matching genes to their TSS's
gene_tss <- read.table('gene_tss.txt', header = TRUE, sep = '\t')

# Get data.frame matching diseases/traits to gbins
snp_gbin <- read.delim('snp_gbin.txt', header = TRUE, sep = '\t')

# Get data.frame matching gbins to diseases/traits
gbin_snp <- read.delim('gbin_snp.txt', header = TRUE, sep = '\t')

# Get data.frame of overall sample prediction means
sample_avgs <- read.table('sample_avgs.txt', header = TRUE, sep = '\t')

# Set recount3 url
recount3_url <- "https://recount-opendata.s3.amazonaws.com/recount3/release"

# Set hdf5 file name
#h5_files <- c("BIRD_predictions.h5")

# Read h5 files mapping to studies
h5_studies <- read.table('h5_studies.txt', header=TRUE, sep='\t')

# Read samples table
all_samples_df <- read.table('samples_df.txt', header=TRUE, sep='\t')

# Read projects table
proj_df <- read.delim('proj_df.txt')
...
server <- function(input, output, session) {
  ...
  get_pred_vec <- function(proj_id, sample_id, range) {
    ...
    for (chr in levels(seqnames(range))) {
      ...
      pred_arr <- h5read(h5_file, paste(proj_id, sample_id, chr, sep = "/"))
